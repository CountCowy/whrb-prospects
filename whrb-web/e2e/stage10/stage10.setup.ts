...
setup('stage10 · authenticate admin + synthetic rep', async ({ page, baseURL }) => {
  if (!baseURL) throw new Error('baseURL must be set in playwright.config.ts');
  // Mirror the Stage 9 tolerance: CI runs without a Stage 10 plant, so the
  // snapshot will be absent — skip gracefully. The stage10 specs also guard
  // with snapshotExists() at the test level.
  try {
    const snap = loadSnapshot();

    // Defensive: reset the synthetic rep's role + deactivation state so
    // repeated Playwright runs against local state remain deterministic.
    ...
    if (msg.includes('stage10_snapshot.json')) {
      setup.skip(true, 'Stage 10 plant snapshot missing — stage10 specs will skip.');
      return;
    }
    throw err;
  }
