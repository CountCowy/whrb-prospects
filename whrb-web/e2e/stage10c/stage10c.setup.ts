...
setup('stage10c · authenticate admin + synthetic rep', async ({ page, baseURL }) => {
  if (!baseURL) throw new Error('baseURL must be set in playwright.config.ts');
  if (!snapshotExists()) {
    setup.skip(true, 'Stage 10c snapshot missing — stage10c specs will skip.');
    return;
  }
  const snap = loadSnapshot();

  // Defensive: reset the synthetic rep's role so repeated runs are deterministic.
