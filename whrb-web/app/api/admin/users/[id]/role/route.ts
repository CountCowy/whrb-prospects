import { NextResponse } from 'next/server';
import { z } from 'zod';
import { createServiceClient } from '@/lib/supabase/service';
import { getAuthed } from '@/lib/server/authz';
import { logEvent } from '@/lib/logging/server';

export const runtime = 'nodejs';

const Body = z.object({ role: z.enum(['admin', 'rep']) });

type RouteParams = { params: Promise<{ id: string }> };

export async function PATCH(req: Request, { params }: RouteParams) {
  const { id } = await params;

  const authz = await getAuthed();
  if (authz.kind === 'unauth') {
    return NextResponse.json({ error: 'Unauthenticated.' }, { status: 401 });
  }
  if (authz.user.role !== 'admin') {
    return NextResponse.json({ error: 'Admin role required.' }, { status: 403 });
  }

  let json: unknown;
  try {
    json = await req.json();
  } catch {
    return NextResponse.json({ error: 'Invalid JSON body.' }, { status: 400 });
  }
  const parsed = Body.safeParse(json);
  if (!parsed.success) {
    const issue = parsed.error.issues[0];
    return NextResponse.json(
      { error: `Invalid payload: ${issue.path.join('.') || '<root>'} — ${issue.message}` },
      { status: 400 },
    );
  }

  // Guard: cannot demote yourself if you are the only admin.
  const service = createServiceClient();
  if (parsed.data.role === 'rep' && id === authz.user.id) {
    const { count } = await service
      .from('profiles')
      .select('*', { count: 'exact', head: true })
      .eq('role', 'admin');
    if ((count ?? 0) <= 1) {
      return NextResponse.json(
        { error: 'Cannot demote the last admin.' },
        { status: 409 },
      );
    }
  }

  const { data, error } = await service
    .from('profiles')
    .update({ role: parsed.data.role })
    .eq('id', id)
    .select('id, email, role')
    .maybeSingle();
  if (error) {
    await logEvent({
      source: 'web_server',
      level: 'error',
      category: 'api_exception',
      message: 'role PATCH failed',
      context: { target: id, code: error.code, message: error.message },
      userId: authz.user.id,
    });
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
  if (!data) {
    return NextResponse.json({ error: 'User not found.' }, { status: 404 });
  }

  await logEvent({
    source: 'web_server',
    level: 'info',
    category: 'admin_user_role_changed',
    message: `role → ${parsed.data.role} for ${data.email}`,
    context: { target: id, new_role: parsed.data.role },
    userId: authz.user.id,
  });
  return NextResponse.json(data);
}
