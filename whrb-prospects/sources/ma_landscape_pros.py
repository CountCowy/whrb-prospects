"""Stage T7 — MA Association of Landscape Professionals.
...
LIVE_URL = "https://www.malp.org/find-a-pro"


def _emit_from_html(html: str) -> list[dict]:
    soup = BeautifulSoup(html, "html.parser")
    rows: list[dict] = []
    for member in soup.select("article.member, .pro-card, .member-card"):
        ...
        rows.append(
            common.build_row(
                ...
                company_name=name,
                category="home_services/landscaping",
                zip_code=zip_code,
                phone=phone,
                website=website,
                tier="C",
                ...
                cadence="seasonal_spring",
                pipeline_notes="ma_landscape_pros: MALP member",
            )
        )
