#!/usr/bin/env python3
"""Apply Stage T6's 016_t6_vocab.sql migration to WHRB dev.

Mirrors apply_t5_migration.py. The DDL uses ``insert ... on conflict do
nothing`` for every row, so this script is safe to re-run.

Usage:
    .venv/bin/python scripts/apply_t6_migration.py
    .venv/bin/python scripts/apply_t6_migration.py --dry-run
    .venv/bin/python scripts/apply_t6_migration.py --rollback
"""
...
MIGRATION = (
    REPO_ROOT / "whrb-web" / "supabase" / "migrations" / "016_t6_vocab.sql"
)
ROLLBACK = (
    REPO_ROOT / "whrb-web" / "supabase" / "migrations" / "016_rollback.sql"
)
...
def main() -> int:
    ...
    ap.add_argument(
        "--rollback",
        action="store_true",
        help="Apply 016_rollback.sql instead of 016_t6_vocab.sql.",
    )
    args = ap.parse_args()
