#!/usr/bin/env python3
"""Apply 018_ad_orders.sql migration to WHRB dev.

Mirrors apply_t7_migration.py. The DDL uses ``create table if not
exists``, ``create or replace function``, and ``drop trigger if
exists`` + ``create trigger`` so re-running on a healthy DB is a
no-op (the org_settings seed insert uses ``on conflict do nothing``).

Usage:
    .venv/bin/python scripts/apply_ad_orders_migration.py
    .venv/bin/python scripts/apply_ad_orders_migration.py --dry-run
    .venv/bin/python scripts/apply_ad_orders_migration.py --rollback
"""
...
MIGRATION = (
    REPO_ROOT / "whrb-web" / "supabase" / "migrations" / "018_ad_orders.sql"
)
ROLLBACK = (
    REPO_ROOT / "whrb-web" / "supabase" / "migrations" / "018_rollback.sql"
)
...
def main() -> int:
    ...
    ap.add_argument(
        "--rollback",
        action="store_true",
        help="Apply 018_rollback.sql instead of 018_ad_orders.sql.",
    )
    args = ap.parse_args()
