#!/usr/bin/env python3
"""Apply Stage T3's 009_tag_triggers.sql migration to WHRB dev.

Mirrors apply_t2_migration.py. The DDL uses ``add column if not exists`` /
``create or replace function`` / ``drop trigger if exists`` everywhere, so
this script is safe to re-run.

Usage:
    .venv/bin/python scripts/apply_t3_migration.py
    .venv/bin/python scripts/apply_t3_migration.py --dry-run
    .venv/bin/python scripts/apply_t3_migration.py --rollback
"""
...
MIGRATION = (
    REPO_ROOT / "whrb-web" / "supabase" / "migrations" / "009_tag_triggers.sql"
)
ROLLBACK = (
    REPO_ROOT / "whrb-web" / "supabase" / "migrations" / "009_rollback.sql"
)
...
def main() -> int:
    ...
    ap.add_argument(
        "--rollback",
        action="store_true",
        help="Apply 009_rollback.sql instead of 009_tag_triggers.sql.",
    )
    args = ap.parse_args()
