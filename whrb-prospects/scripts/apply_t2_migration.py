#!/usr/bin/env python3
"""Apply Stage T2's 008_daypart_view.sql migration to WHRB dev.

Mirrors ``apply_t1_migration.py``. The DDL uses ``if not exists`` /
``drop ... if exists`` / ``create or replace`` everywhere, so this script
is safe to re-run.

Usage:
    .venv/bin/python scripts/apply_t2_migration.py
    .venv/bin/python scripts/apply_t2_migration.py --dry-run
    .venv/bin/python scripts/apply_t2_migration.py --rollback
"""
...
MIGRATION = (
    REPO_ROOT / "whrb-web" / "supabase" / "migrations" / "008_daypart_view.sql"
)
ROLLBACK = (
    REPO_ROOT / "whrb-web" / "supabase" / "migrations" / "008_rollback.sql"
)
...
def main() -> int:
    ...
    ap.add_argument(
        "--rollback",
        action="store_true",
        help="Apply 008_rollback.sql instead of 008_daypart_view.sql.",
    )
    args = ap.parse_args()
