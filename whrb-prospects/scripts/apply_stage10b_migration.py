#!/usr/bin/env python3
"""Apply Stage 10b's 004_prospects_notes_internal.sql migration to WHRB dev.

Mirrors apply_stage9_migration.py. The SQL uses `add column if not exists`
so the script is safe to re-run.

Usage:
    .venv/bin/python scripts/apply_stage10b_migration.py
    .venv/bin/python scripts/apply_stage10b_migration.py --dry-run
"""
...
MIGRATION = (
    REPO_ROOT / "whrb-web" / "supabase" / "migrations" / "004_prospects_notes_internal.sql"
)
...
def _conn_from_env():
    ...
    if not project_ref or not password:
        ...
        sys.exit(2)
    # Supabase moved the project to the aws-1-us-west-2 pooler. The old
    # direct host returns IPv6-only DNS which is unreachable from this
    # network. Try the current pooler first; fall back to legacy direct.
    pooler_dsn = (
...
def main() -> int:
    ...
    try:
        with conn, conn.cursor() as cur:
            cur.execute(sql)
        print("OK: 004_prospects_notes_internal.sql applied.")
    finally:
        conn.close()
    return 0
