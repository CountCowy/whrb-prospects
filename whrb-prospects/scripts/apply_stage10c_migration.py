#!/usr/bin/env python3
"""Apply Stage 10c's 005_pipeline_runs_github_run_id.sql migration to WHRB dev.

Mirrors apply_stage10b_migration.py. The SQL uses `add column if not exists`
so the script is safe to re-run.

Usage:
    .venv/bin/python scripts/apply_stage10c_migration.py
    .venv/bin/python scripts/apply_stage10c_migration.py --dry-run
"""
...
def main() -> int:
    ...
    try:
        with conn, conn.cursor() as cur:
            cur.execute(sql)
        print("OK: 005_pipeline_runs_github_run_id.sql applied.")
    finally:
        conn.close()
    return 0
