#!/usr/bin/env python3
"""Apply Stage T4's 011_instrumentation.sql migration to WHRB dev.

Mirrors apply_t3_migration.py. The DDL uses ``create table if not exists`` /
``add column if not exists`` / ``create index if not exists`` /
``create or replace function`` everywhere, so this script is safe to re-run.

Usage:
    .venv/bin/python scripts/apply_t4_migration.py
    .venv/bin/python scripts/apply_t4_migration.py --dry-run
    .venv/bin/python scripts/apply_t4_migration.py --rollback
"""
...
MIGRATION = (
    REPO_ROOT / "whrb-web" / "supabase" / "migrations" / "011_instrumentation.sql"
)
ROLLBACK = (
    REPO_ROOT / "whrb-web" / "supabase" / "migrations" / "011_rollback.sql"
)
...
def main() -> int:
    ...
    ap.add_argument(
        "--rollback",
        action="store_true",
        help="Apply 011_rollback.sql instead of 011_instrumentation.sql.",
    )
    args = ap.parse_args()
