#!/usr/bin/env python3
"""Apply 010_prospect_contact_emails.sql to WHRB dev.

Mirrors apply_t3_migration.py. The DDL uses ``create table if not exists``
+ ``add column if not exists`` + ``create or replace function`` +
``drop trigger if exists`` everywhere, so this script is safe to re-run.

Usage:
    .venv/bin/python scripts/apply_multi_email_migration.py
    .venv/bin/python scripts/apply_multi_email_migration.py --dry-run
    .venv/bin/python scripts/apply_multi_email_migration.py --rollback
"""
...
MIGRATION = (
    ...
    / "supabase"
    / "migrations"
    / "010_prospect_contact_emails.sql"
)
ROLLBACK = (
    REPO_ROOT / "whrb-web" / "supabase" / "migrations" / "010_rollback.sql"
)
...
def main() -> int:
    ...
    ap.add_argument(
        "--rollback",
        action="store_true",
        help="Apply 010_rollback.sql instead of 010_prospect_contact_emails.sql.",
    )
    args = ap.parse_args()
