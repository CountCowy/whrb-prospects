#!/usr/bin/env python3
"""Apply Stage 7's 001_prospect_update_guard.sql migration to WHRB dev.

Mirrors apply_migration.py but targets the single Stage 7 migration file and
is safe to re-run (the SQL itself uses `create or replace function` and
`drop trigger if exists`).

Usage:
    .venv/bin/python scripts/apply_stage7_migration.py
    .venv/bin/python scripts/apply_stage7_migration.py --dry-run
"""
...
MIGRATION = (
    REPO_ROOT / "whrb-web" / "supabase" / "migrations" / "001_prospect_update_guard.sql"
)
...
def main() -> int:
    ...
    try:
        with conn, conn.cursor() as cur:
            cur.execute(sql)
        print("OK: 001_prospect_update_guard.sql applied.")
    finally:
        conn.close()
    return 0
